...
class Generator(nn.Module):
  def __init__(self):
    ...
    self.FCin = self._block(z_dim, 512, 4, 1, 0, False)
    self.FC1 = self._block(512, 256, 4, 2, 1, False)
    self.FC2 = self._block(256, 128, 4, 2, 1, False)
    self.FC3 = self._block(128, 64, 4, 2, 1, False)
  ...
  # hidden layer
  def _block(self, in_fea, out_fea, k_size, stride, padding, bias=False):
    return nn.Sequential(
       nn.ConvTranspose2d(in_fea, out_fea, kernel_size=k_size, stride=stride, padding=padding, bias=bias),
